...
from database.models import Token as TokenModel
...
def get_user_token(
    ...
    x_api_token: Optional[str] = Header(
        ...
        description=(
            ...
            "when you generate a new token from `/v1/users/me/newtoken` endpoint."
        ),
        min_length=32,
        max_length=32,
    ...
    if not x_api_token:
        raise_error(
            400,
            message="X-Token header is required for authentication."
        )
    try:
        db_utils.verify_token(db, x_api_token)
    except TokenNotFoundError:
        raise_error(401, message="Invalid API token.")
    response.headers["X-API-Token"] = x_api_token
    return x_api_token
